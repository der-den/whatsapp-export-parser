...
class PDFAttachmentGenerator:
    def __init__(self, output_dir: str, unzip_dir: Optional[str] = None):
            ...
            if msg.is_attachment and msg.exists_in_export and msg.attachment_file:
                try:
                    ...
                    if os.path.exists(full_path):
                        ...
                        if pdf_path:
                            print(f"Generated PDF for attachment: {os.path.basename(pdf_path)}")
                            pdfs_generated += 1
    ...
    def _create_image_pdf(self, image_path: str, output_pdf: str, metadata: Optional[Dict] = None) -> Optional[str]:
        ...
        try:
            ...
            # Add metadata
            if metadata:
                elements.append(Paragraph("Metadata:", self.styles['Heading2']))
