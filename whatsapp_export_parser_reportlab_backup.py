...
class WhatsAppExportParser:
    ...
    def parse_message_line(self, line: str) -> Optional[ChatMessage]:
        ...
        try:
            ...
            try:
                ...
                debug_print(f"Demojized content: {content}")
            except Exception as e:
    ...
    def find_attachment_file(self, filename):
        ...
        debug_print(f"Looking for attachment: {filename}")
        
        # First try exact match
        for root, _, files in os.walk(self.extract_path):
            if filename in files:
                file_path = os.path.join(root, filename)
                debug_print(f"Found exact match: {file_path}")
                return file_path
        ...
        for root, _, files in os.walk(self.extract_path):
            for file in files:
                if file.lower() == filename_lower:
                    ...
                    debug_print(f"Found case-insensitive match: {file_path}")
                    return file_path
        ...
        for root, _, files in os.walk(self.extract_path):
            for file in files:
                ...
                if clean_file == clean_filename:
                    file_path = os.path.join(root, file)
                    debug_print(f"Found match without special chars: {file_path}")
                    return file_path
        
        debug_print(f"No matching file found for: {filename}")
        return None
    ...
    def check_media_files(self):
        ...
        for message in self.chat_messages:
            if message.is_attachment and message.attachment_file:
                debug_print(f"Processing attachment from message: {message.attachment_file}")
                ...
                for root, _, files in os.walk(self.extract_path):
                    if message.attachment_file in files:
                        ...
                        if file_path not in self.media_files:
                            self.media_files.append(file_path)
                            debug_print(f"  Found referenced file: {file_path}")
                        break
        ...
        for root, _, files in os.walk(self.extract_path):
            for file in files:
                if any(file.lower().endswith(ext) for ext in media_extensions):
                    ...
                    if file_path not in self.media_files:
                        self.media_files.append(file_path)
                        debug_print(f"Found additional media file: {file}")
        ...
        for file_path in self.media_files:
            try:
                ...
                debug_print(f"\nMedia file details:")
                debug_print(f"  Name: {file_name}")
                debug_print(f"  Path: {file_path}")
                debug_print(f"  Size: {self.format_size(file_size)}")
                debug_print(f"  MD5: {md5_hash}")
                debug_print(f"  Duration: {duration}")
                ...
                debug_print(f"Error processing media file {file_path}: {str(e)}")
                continue
        
        debug_print(f"\nFound {len(self.media_files)} media files in total")

    def get_media_duration(self, file_path):
        debug_print(f"Getting duration for: {file_path}")
        try:
            ...
            debug_print(f"  Error getting duration: {str(e)}")
            return "N/A"
    ...
    def generate_media_pdf(self, output_dir):
        ...
        debug_print(f"Generating media PDF: {media_pdf_path}")
    ...
    def setup_pdf_fonts(self):
        """Setup fonts for PDF generation with proper Unicode support"""
        # Try to find a suitable Unicode font
        font_paths = [
            ...
            # Noto Sans - good Unicode and emoji support
        ...
        for font_path in font_paths:
            if os.path.exists(font_path):
                try:
                    ...
                    debug_print(f"Registered font: {font_name}")
                except Exception as e:
                    debug_print(f"Failed to register font {font_path}: {str(e)}")
        
        if not registered_fonts:
            debug_print("Warning: No Unicode fonts found, falling back to Helvetica")
            return "Helvetica"
            
        # Create font mapping for emoji fallback
        if len(registered_fonts) > 1:
            try:
                # Create a mapping that tries emoji fonts first, then regular fonts
                mapping = pdfmetrics.UnicodeCMap('utf8')
                for font in registered_fonts:
                    mapping.addMapping(font)
                debug_print("Created font mapping for emoji support")
            except Exception as e:
                debug_print(f"Failed to create font mapping: {str(e)}")
        
        return registered_fonts[0]  # Return the first successfully registered font
    ...
    def generate_pdf(self, output_path):
        ...
        try:
            ...
            # Add custom chat styles to the stylesheet
            ...
            if zip_info:
                print(f"Processing ZIP file: {zip_info['name']} ({zip_info['size']})")
                elements.append(Paragraph(f"File: {zip_info['name']}", subtitle_style))
                elements.append(Paragraph(f"Size: {zip_info['size']}", subtitle_style))
                elements.append(Paragraph(f"Date: {zip_info['date']}", subtitle_style))
                elements.append(Paragraph(f"MD5: {zip_info['md5']}", subtitle_style))
                elements.append(Spacer(1, 20))
            ...
            if self.chat_messages:
                ...
                elements.append(Paragraph("Chat Statistics:", subtitle_style))
                elements.append(Paragraph(f"Messages: {len(self.chat_messages):,}", subtitle_style))
                elements.append(Paragraph(f"Participants: {unique_senders}", subtitle_style))
                elements.append(Paragraph(f"Attachments: {attachment_count:,}", subtitle_style))
            ...
            for i, message in enumerate(self.chat_messages, 1):
                ...
                # Add sender name
                elements.append(Paragraph(message.sender, style))
                ...
                if message.is_attachment and message.exists_in_export:
                    ...
                    if attachment_file:
                        ...
                        if file_path and self.is_image_file(file_path):
                            debug_print(f"Processing image: {file_path}")
